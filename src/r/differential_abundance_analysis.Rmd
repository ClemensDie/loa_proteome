...
data_adj <- data[(data_adj$infection=="loa") | (data_adj$infection=="healthy controls"), ]

data <- data[which((data$eos_abs < 0.5 & data$`disease severity` == "healthy control") | data$`disease severity` != "healthy control"), ]
data_adj <- data_adj[which((data_adj$eos_abs < 0.5 & data_adj$`disease severity` == "healthy control") | data_adj$`disease severity` != "healthy control"), ]
...
data_adj[data_adj$infection == "healthy controls", "infection"] <- "CTRL"
```

## Differential Expression Analysis

```{r}
table(data["infection"])
```
...
for (i in 1:length(clinical_symptoms_bin)) {
  ...
  data_symp <- data_adj[
    (data_adj[c] == "Ctrl asympt.") | (data_adj[c] == "Ctrl sympt.") | (data_adj[c] == "EW asympt.") | (data_adj[c] == "EW sympt."),]
  data_symp[data_symp[c] == "Ctrl asympt.", c] = 1
  data_symp[data_symp[c] == "Ctrl sympt.", c] = 2
  data_symp[data_symp[c] == "EW asympt.", c] = 3
  data_symp[data_symp[c] == "EW sympt.", c] = 4
