...
```{r warning = FALSE}
# load data
setwd("C:/Users/cleme/OneDrive - Charité - Universitätsmedizin Berlin/Projects/02_NTD_CHARITE/NTD")
...
adj_file_path <- "data/analysis/loa/loa_data_adj.csv"
data_adj <- read.table(adj_file_path, sep=",", header = T, check.names = F)
...
# data_adj[, common_uniprot] <- as.data.frame(sapply(data_adj[, common_uniprot], as.numeric))
...
Since we observe age differences between the INF and LN group, as well as
age and sex differences between LN and the severity levels. We use matching to 
balance age and sex effects in our cohort. Therefore we balance every comparisson 
separately.

1: LN (m=201) vs INF (274)
We match the larger INF group to the smaller LN group.

2: MF vs EW (1), MF vs EWMF (2), Ew vs EWMF (3)

3: MF vs LN (1), EWMF vs LN (2), EWMF vs LN (3)

4: Men vs Women in MF, LN, EW (EWMF already perfectly balanced)  only age group matching
...
ggplot(data_m, aes(x = disease_severity, fill = factor(age_group))) +
  ...
  labs(title = "Before Matching", 
       ...
       y = "Frequency", 
       fill = "Age Groups") +
  scale_fill_manual(values = c("#acd8a7", "#46923c", "#276221"), labels = c("<34", ">34 & <60", ">60")) +
  theme_minimal()
...
write.csv(data_matched, "data/analysis/loa/matched/data_matched_inf_ln.csv")
```
...
data_matched <- match.data(m)
data_matched$disease_severity <- ifelse(data_matched$disease_severity == 1, "MF", "EW")
...
write.csv(data_matched, "data/analysis/loa/matched/data_matched_mf_ew.csv")
```
...
data_matched <- match.data(m)
data_matched$disease_severity <- ifelse(data_matched$disease_severity == 1, "MF", "EWMF")
...
write.csv(data_matched, "data/analysis/loa/matched/data_matched_mf_ewmf.csv")
```
... followed by MF and EWMF ...
...
data_matched <- match.data(m)
data_matched$disease_severity <- ifelse(data_matched$disease_severity == 1, "EWMF", "EW")
...
write.csv(data_matched, "data/analysis/loa/matched/data_matched_ew_ewmf.csv")
```

Since we are comparing symptomatic to asymptomatic LN and EW, 
we investigated differences in age_group and sex distributions. Since
some of the symptoms have very small sample sizes and distribution do not 
...
whether there are immunological differences between men and women for 
EW or MF.
...
write.csv(data_matched, "data/analysis/loa/matched/data_matched_mf_sex.csv")
```
```{r warning=FALSE}
data_m <- data
...
# write.csv(data_matched, "data/analysis/loa/matched/data_matched_ewmf_sex.csv")
```
Since Balancding does not lead to closer distributions, we do not match the age 
groups in the EWMF group.
...
write.csv(data_matched, "data/analysis/loa/matched/data_matched_ew_sex.csv")
```
...
write.csv(data_matched, "data/analysis/loa/matched/data_matched_ln_sex.csv")
```


## Differential Expression Analysis

Furthermore we conduct a differential expression analyis for the following 
contrasts:
...
write.csv(res_infection, "results/loaloa/tables/dea_limma/ln_infected.csv")
```
### Severity Groups
...
write.csv(res, "results/loaloa/tables/dea_limma/mf_ew.csv")
```
```{r warning=FALSE}
### Severity Groups
setwd("C:/Users/cleme/OneDrive - Charité - Universitätsmedizin Berlin/Projects/02_NTD_CHARITE/NTD")
...
res <- topTable(fit3, n=Inf, adjust="BH", coef = 1)
write.csv(res, "results/loaloa/tables/dea_limma/mf_ewmf.csv")
```
```{r warning=FALSE}
### Severity Groups
setwd("C:/Users/cleme/OneDrive - Charité - Universitätsmedizin Berlin/Projects/02_NTD_CHARITE/NTD")
...
res <- topTable(fit3, n=Inf, adjust="BH", coef = 1)
write.csv(res, "results/loaloa/tables/dea_limma/ew_ewmf.csv")
```
...
data_mf_sex[,colnames(data_mf_sex) %in% common_uniprot] <- as.data.frame(sapply(data_mf_sex[,colnames(data_mf_sex) %in% common_uniprot], as.numeric))
fit <- lmFit(t(data_mf_sex[,colnames(data_mf_sex) %in% common_uniprot]), design = design)
...
res <- topTable(fit3, n=Inf, adjust="BH", coef = 1)
write.csv(res, "results/loaloa/tables/dea_limma/mf_sex.csv")
```
```{r warning=FALSE}
setwd("C:/Users/cleme/OneDrive - Charité - Universitätsmedizin Berlin/Projects/02_NTD_CHARITE/NTD")
...
data_ewmf_sex[,colnames(data_ewmf_sex) %in% common_uniprot] <- as.data.frame(sapply(data_ewmf_sex[,colnames(data_ewmf_sex) %in% common_uniprot], as.numeric))
fit <- lmFit(t(data_ewmf_sex[,colnames(data_ewmf_sex) %in% common_uniprot]), design = design)
...
res <- topTable(fit3, n=Inf, adjust="BH", coef = 1)
write.csv(res, "results/loaloa/tables/dea_limma/ewmf_sex.csv")
```
```{r warning=FALSE}
setwd("C:/Users/cleme/OneDrive - Charité - Universitätsmedizin Berlin/Projects/02_NTD_CHARITE/NTD")
...
data_ew_sex[,colnames(data_ew_sex) %in% common_uniprot] <- as.data.frame(sapply(data_ew_sex[,colnames(data_ew_sex) %in% common_uniprot], as.numeric))
fit <- lmFit(t(data_ew_sex[,colnames(data_ew_sex) %in% common_uniprot]), design = design)
...
res <- topTable(fit3, n=Inf, adjust="BH", coef = 1)
write.csv(res, "results/loaloa/tables/dea_limma/ew_sex.csv")
```
```{r warning=FALSE}
setwd("C:/Users/cleme/OneDrive - Charité - Universitätsmedizin Berlin/Projects/02_NTD_CHARITE/NTD")
...
data_ln_sex[,colnames(data_ln_sex) %in% common_uniprot] <- as.data.frame(sapply(data_ln_sex[,colnames(data_ln_sex) %in% common_uniprot], as.numeric))
fit <- lmFit(t(data_ln_sex[,colnames(data_ln_sex) %in% common_uniprot]), design = design)
...
res <- topTable(fit3, n=Inf, adjust="BH", coef = 1)
write.csv(res, "results/loaloa/tables/dea_limma/ln_sex.csv")
```

## Trend Analysis of control and Ew positive patients

Since there are differences in the age and sex distribution between the 
asymptomatic and symptomatic group, we perform this analysis on data, that
was adjusted by linear modeling (protAbund ~ sex + age) and without to dompare the differences. 
Residuals + median protein abundance were used for subsequent analysis.
...
# Export  
write.xlsx(df_res_list, "results/loaloa/tables/sympt_stat_analysis_unadjForSexAndAge.xlsx")
```
