...
4: Women vs Men in MF, LN, EW (EWMF already perfectly balanced)  only age group matching
...
data_m$infection <- ifelse(data_m_1$infection == "LN", 1, 0)


# data_m_1$infection <- as.numeric(as.factor(data_m_1$infection))-1

m <- matchit(
  infection ~ age_group + sex, data = data_m_1, method = "nearest", ratio=1
)
...
ggplot(data, aes(x = infection, fill = factor(age_group))) +
  geom_histogram(stat = "count", position = "fill", color = "black") +
  labs(title = "Histogram of Age Groups by Infection Status", 
...
```
 ...
 Next, we match between the severity groups
...
design_infection <- design_infection[,c("LN", "loa")]
