...
# common uniprot ids
common_uniprot <- colnames(data)[98:371]
...
data <- data[(data$infection=="loa") | (data$infection=="LN"), ]
data_adj <- data[(data_adj$infection=="loa") | (data_adj$infection=="LN"), ]

data <- data[which((data$eos_abs < 0.5 & data$`disease severity` == "LN") | data$`disease severity` != "LN"), ]
data_adj <- data_adj[which((data_adj$eos_abs < 0.5 & data_adj$`disease severity` == "LN") | data_adj$`disease severity` != "LN"), ]
...
was adjusted by linear modeling (protAbund ~ sex + age). 
...
with adjusted data for sex and age

```{r}
setwd("C:/Users/cleme/OneDrive - Charité - Universitätsmedizin Berlin/Projects/02_NTD_CHARITE/NTD")

data_adj <- as.data.frame(data_adj)
...
for (i in 1:length(clinical_symptoms_bin)) {
  ...
  data_symp <- data_adj[
    (data_adj[c] == "LN asympt.") | (data_adj[c] == "LN sympt.") | (data_adj[c] == "EW asympt.") | (data_adj[c] == "EW sympt."),]
  data_symp[data_symp[c] == "LN asympt.", c] = 1
  data_symp[data_symp[c] == "LN sympt.", c] = 2
  data_symp[data_symp[c] == "EW asympt.", c] = 3
  data_symp[data_symp[c] == "EW sympt.", c] = 4
...
# Export  
write.xlsx(df_res_list, "results/loaloa/tables/sympt_stat_analysis_adjForSexAndAge.xlsx")
```
